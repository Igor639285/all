...
class RespectStorage:
    ...
    def top_chat_leaders(self, chat_id: int, limit: int = 10) -> list[tuple[str, int]]:
        rows = self.conn.execute(
            """
            SELECT username, CAST(respect AS INTEGER) AS respect
            FROM users
            WHERE chat_id=?
            ORDER BY CAST(respect AS INTEGER) DESC, username ASC
            LIMIT ?
            """,
            (chat_id, limit),
        ).fetchall()
        return [(str(r["username"]), int(r["respect"])) for r in rows]

    def top_global_leaders(self, limit: int = 10) -> list[tuple[str, int]]:
        rows = self.conn.execute(
            """
            SELECT user_id, MAX(username) AS username, SUM(CAST(respect AS INTEGER)) AS total_respect
            FROM users
            GROUP BY user_id
            ORDER BY total_respect DESC, username ASC
            LIMIT ?
            """,
            (limit,),
        ).fetchall()
        return [(str(r["username"]), int(r["total_respect"])) for r in rows]
...
async def handle_message(update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    if text == ".stats":
        leaders = storage.top_chat_leaders(message.chat.id)
        if not leaders:
            await message.reply_text(phrases["stats_empty"])
            return
        lines = [phrases["stats_title"]]
        for idx, (name, respect) in enumerate(leaders, start=1):
            lines.append(f"{idx}. {name} — {respect}")
        await message.reply_text("\n".join(lines))
        return

    if text == ".glstats":
        leaders = storage.top_global_leaders()
        if not leaders:
            await message.reply_text(phrases["stats_empty"])
            return
        lines = [phrases["glstats_title"]]
        for idx, (name, respect) in enumerate(leaders, start=1):
            lines.append(f"{idx}. {name} — {respect}")
        await message.reply_text("\n".join(lines))
        return
